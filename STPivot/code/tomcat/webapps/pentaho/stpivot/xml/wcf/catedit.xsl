...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:param name="imgpath0" select="'stpivot/style/wcf/catedit'"/>
	...
	<xsl:template match="cat-category">
		<tr>
			<th align="left" class="navi-axis">
				<img src="{$context}/{@imgpath0}/{@icon}" width="9" height="9"/>
				<xsl:text> </xsl:text>
	...
	<xsl:template match="cat-button[@icon]">
		<input border="0" type="image" src="{$context}/{@imgpath0}/{@icon}" name="{@id}" width="9" height="9"/>
		<xsl:text> </xsl:text>
	</xsl:template>
	
	<xsl:template match="cat-button">
		<img src="{$context}/{@imgpath0}/empty.png" width="9" height="9"/>
		<xsl:text> </xsl:text>
	</xsl:template>
