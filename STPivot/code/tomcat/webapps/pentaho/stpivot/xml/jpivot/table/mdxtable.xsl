...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		...
		<td nowrap="nowrap">
			<xsl:choose> 
				<xsl:when test="not(contains('odd,even,rot,gelb,gruen',@style))"> <!-- for any style other than odd,even,rot,gruen,gelb -->
					<xsl:attribute name="class"><xsl:text>cell-red</xsl:text><xsl:text> selectable</xsl:text></xsl:attribute> <!-- hack to force cascade of non-color styles -->
					<xsl:attribute name="style"><xsl:text>background-color:</xsl:text><xsl:value-of select="@style"/></xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="class"><xsl:text>cell-</xsl:text><xsl:value-of select="@style" /><xsl:text> selectable</xsl:text></xsl:attribute>
				</xsl:otherwise>
			</xsl:choose>
	...
	<xsl:template match="popup-menu">
		<a href="#" onMouseover="cssdropdown.dropit(this, event, '{@id}')" onclick="return false">
			<xsl:value-of select="@label" />
		</a>
	...
	<xsl:template match="popup-item">
		<a href="{@href}" style="padding-left: {@level}em" target="_blank">
			<xsl:value-of select="@label" />
		</a>
	</xsl:template>
	<!-- end popup menu  -->
