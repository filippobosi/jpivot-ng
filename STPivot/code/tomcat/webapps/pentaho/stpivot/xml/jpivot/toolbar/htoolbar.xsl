...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:param name="imgpath" select="'stpivot/style/jpivot/toolbar'"/>
	
	<xsl:template match="tool-bar">
		<table border="0" cellspacing="2" cellpadding="0" id="{$renderId}">
			<tr>
				<xsl:apply-templates/>
			</tr>
		</table>
	...
	<xsl:template match="tool-button">
		<td>
			<input type="image" name="{@id}" src="{$context}/{$imgpath}/{@img}.png" border="0" title="{@title}" width="24" height="24" class="imgButton"/>
		</td>
	</xsl:template>
